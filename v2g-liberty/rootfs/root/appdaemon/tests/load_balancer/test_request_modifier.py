...
from apps.load_balancer.request_modifier import (
    RegisterAddress,
    RequestModifier,
    int16_to_uint16,
)
...
def test_init(request_modifier):
    assert request_modifier.limit == 0


def test_set_limit_to_setpoint(proxy, request_modifier, callback):
    ...
    proxy.client.write_register.assert_called_once_with(
        RegisterAddress.SET_POWER_SETPOINT, 2000
    )


def test_set_limit_to_negative_setpoint(proxy, request_modifier, callback):
    request_modifier.add_listener("limit", callback)
    request_modifier._requested_power_setpoint = -2000
    ...
    callback.assert_called_once_with(-2000)
    proxy.client.write_register.assert_called_once_with(
        RegisterAddress.SET_POWER_SETPOINT, int16_to_uint16(-2000)
    )


def test_set_limit_above_setpoint(proxy, request_modifier, callback):
    request_modifier.add_listener("limit", callback)
    ...
    callback.assert_called_once_with(2000)
    proxy.client.write_register.assert_called_once_with(
        RegisterAddress.SET_POWER_SETPOINT, 1000
    )


def test_set_limit_above_negative_setpoint(proxy, request_modifier, callback):
    request_modifier.add_listener("limit", callback)
    request_modifier._requested_power_setpoint = -1000
    ...
    callback.assert_called_once_with(-2000)
    proxy.client.write_register.assert_called_once_with(
        RegisterAddress.SET_POWER_SETPOINT, int16_to_uint16(-1000)
    )


def test_set_limit_below_setpoint(proxy, request_modifier, callback):
    request_modifier.add_listener("limit", callback)
    request_modifier._requested_power_setpoint = 2000
...
def test_set_limit_below_negative_setpoint(proxy, request_modifier, callback):
    request_modifier.add_listener("limit", callback)
    request_modifier._requested_power_setpoint = -2000
...
def test_set_setpoint_below_limit(request_modifier):
    request = WriteSingleRegisterRequest(
        address=RegisterAddress.SET_POWER_SETPOINT, registers=[1000]
    )
...
def test_set_negative_setpoint_below_limit(request_modifier):
    ...
    request = WriteSingleRegisterRequest(
        address=RegisterAddress.SET_POWER_SETPOINT, registers=[minus_thousand]
    )
    request_modifier.limit = 2000
...
def test_set_setpoint_above_limit(request_modifier):
    request = WriteSingleRegisterRequest(
        address=RegisterAddress.SET_POWER_SETPOINT, registers=[2000]
    )
...
def test_set_negative_setpoint_above_limit(request_modifier):
    ...
    request = WriteSingleRegisterRequest(
        address=RegisterAddress.SET_POWER_SETPOINT, registers=[minus_two_thousand]
    )
    request_modifier.limit = 1000
