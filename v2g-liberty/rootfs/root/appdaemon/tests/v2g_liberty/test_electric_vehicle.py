...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Test EV", 5, 80, 150, 20, 80, None, 0.8, 150, 20, 80),
        # Battery capacity too high
        ("Test EV", 250, 80, 150, 20, 80, None, 0.8, 150, 20, 80),
        ...
        # Consumption too low
        ("Test EV", 50, 80, 50, 20, 80, 50, 0.8, None, 20, 80),
        # Consumption too high
        ("Test EV", 50, 80, 500, 20, 80, 50, 0.8, None, 20, 80),
        # Min SoC too low
...
def test_initialise_ev(
    ...
    assert ev.min_soc == expected_min_soc
    assert ev.max_soc == expected_max_soc
...
async def test_set_soc_emits_remaining_range_event(mock_hass, mock_event_bus):
    ...
    # Assert
    mock_event_bus.emit_event.assert_any_call(
        ev.REMAINING_RANGE_EVENT_NAME,
        remaining_range=ev.remaining_range_km
    )
