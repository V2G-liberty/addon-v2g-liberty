...
class V2Gliberty(hass.Hass):
    ...
    async def __set_soc_prognosis_in_chart(self, chart_line_name: ChartLine, records: dict):
        """ Write or remove SoC prognosis lines in the chart.

            If records = None the SoC line_name line will be removed from the graph,
            ...
            Parameters:
                ...
                records(dict): a dictionary of time (isoformat) + SoC (%) records
                If this is None, it is assumed that only the chart_line_name needs to be 'erased'.
                If there is data in records, it is assumed that the other lines need to be erased.
        ...
        new_state = "SoC prognosis at " + now.isoformat()
    ...
    async def set_next_action(self, v2g_args=None):
        ...
        if charge_mode == "Automatic":
            ...
            if self.connected_car_soc > c.CAR_MIN_SOC_IN_PERCENT and self.in_boost_to_reach_min_soc:
                ...
                await self.__set_soc_prognosis_in_chart(chart_line_name=ChartLine.BOOST, records=None)
            ...
            if self.connected_car_soc >= 100:
                ...
                # How much energy (wh) is needed, taking roundtrip efficiency into account
                # For % /100, for kwh to wh * 1000 results in *10...
                delta_to_max_soc_wh = (100 - self.connected_car_soc) * c.CAR_MAX_CAPACITY_IN_KWH * 10
                ...
                max_charge_now_prognoses.append(dict(time=expected_max_soc_time, soc=100))
                await self.__set_soc_prognosis_in_chart(
                    chart_line_name = ChartLine.MAX_CHARGE_NOW,
