...
class ChartLine(enum.Enum):
    ...
    MAX_CHARGE_NOW = 2  # SoC prognosis based on UI function Max Charge Now (dashed line)
...
class V2Gliberty(hass.Hass):
    ...
    async def __update_charge_mode(self, entity, attribute, old, new, kwargs):
        ...
        self.log(f"__update_charge_mode: Charge mode has changed from '{old_state}' to '{new_state}'")
        ...
        if old_state == 'Automatic' and new_state == 'Max boost now':
            self.log("__update_charge_mode: Charge mode changed from Automatic to Max boost now, "
        ...
        if old_state != 'Stop' and new_state == 'Stop':
            ...
            self.log("__update_charge_mode: Stop charging (if in action) and give control based on chargemode = Stop")
    ...
    async def __clear_all_soc_chart_lines(self):
        await self.set_records_in_chart(chart_line_name = ChartLine.SCHEDULE, records = None)
        await self.set_records_in_chart(chart_line_name = ChartLine.BOOST, records = None)
        await self.set_records_in_chart(chart_line_name = ChartLine.MAX_CHARGE_NOW, records = None)
    ...
    async def set_records_in_chart(self, chart_line_name: ChartLine, records: dict):
        ...
        clear_line_records = dict(records=[dict(time=(now + timedelta(days=7)).isoformat(), soc=None)])
        ...
        if records is None:
            await self.set_state(entity, state=new_state, attributes=clear_line_records)
            self.log(f"set_records_in_chart | chart_line_name '{chart_line_name}' cleared.")
        else:
            result = {'records': records}
            await self.set_state(entity, state=new_state, attributes=result)
            self.log(f"set_records_in_chart | entity: '{entity}', "
            ...
            if chart_line_name in soc_lines:
                self.log(f"set_records_in_chart | chart_line_name '{chart_line_name}' in soc_lines, clear others.")
                for chart_line in soc_lines:
                    if chart_line == chart_line_name:
                        continue
                    entity = f"input_text.{self.chart_line_entity[chart_line]}"
                    await self.set_state(entity, state=new_state, attributes=clear_line_records)
                    self.log(f"set_records_in_chart | chart_line_name '{chart_line}' cleared.")
    ...
    async def set_next_action(self, v2g_args=None):
        ...
        if charge_mode == "Automatic":
            ...
            if self.connected_car_soc < c.CAR_MIN_SOC_IN_PERCENT and not self.in_boost_to_reach_min_soc:
                ...
                delta_to_min_soc_wh = ((c.CAR_MIN_SOC_IN_PERCENT - self.connected_car_soc)
                                       * c.CAR_MAX_CAPACITY_IN_KWH * 10)
            ...
            if self.connected_car_soc >= c.CAR_MAX_CAPACITY_IN_PERCENT:
                ...
                max_charge_now_prognoses = [dict(time=now.isoformat(), soc=self.connected_car_soc)]

                # How much energy (wh) is needed, taking roundtrip efficiency into account
                # For % /100, for kwh to wh * 1000 results in *10...
                delta_to_max_soc_wh = ((c.CAR_MAX_CAPACITY_IN_PERCENT - self.connected_car_soc)
                                       * c.CAR_MAX_CAPACITY_IN_KWH * 10)
                delta_to_max_soc_wh = delta_to_max_soc_wh / (c.ROUNDTRIP_EFFICIENCY_FACTOR ** 0.5)

                # How long will it take to charge this amount with max power, we use ceil to avoid 0 minutes as
                # this would not show in graph.
                minutes_to_reach_max_soc = int(math.ceil((delta_to_max_soc_wh / c.CHARGER_MAX_CHARGE_POWER * 60)))
                expected_max_soc_time = (now + timedelta(minutes=minutes_to_reach_max_soc)).isoformat()
                max_charge_now_prognoses.append(dict(time=expected_max_soc_time, soc=c.CAR_MAX_CAPACITY_IN_PERCENT))
                await self.set_records_in_chart(
